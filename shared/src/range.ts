...
export function reduceRange(
  ...
  return {
    ...
    views: sums.get("total#views") ?? 0,
    uniques: sums.get("total#uniques") ?? 0,
    // Caps bound the payload, not the UI: dashboards collapse long lists client-side
    // ("show all" / CSV export need the rows to exist in the response).
    topPages: rank(sums, "page#", 50),
    topReferrers: rank(sums, "ref#", 50),
    browsers: rank(sums, "browser#", 8),
    os: rank(sums, "os#", 8),
    sizes: rank(sums, "size#", 8),
    utmSources: rank(sums, "utm_source#", 25),
    utmCampaigns: rank(sums, "utm_campaign#", 25),
    utmMediums: rank(sums, "utm_medium#", 25),
    countries: rank(sums, "country#", 250),
