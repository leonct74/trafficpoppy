...
- What to type in the "Add another domain" field: a SUBDOMAIN of a site I already track — stats.my-site.com is the usual choice, but any name under my domain works (insights., analytics., …). Never the bare domain itself (visitors would land on the statistics page instead of my website), and never an unrelated domain I happen to own (the subscription is per domain, and a domain no tracked site lives under would bill without putting anything online — the app blocks both).
