...
describe("useEntitlement", () => {
  it("asks the host per DOMAIN — one paid unit is one domain (§12)", async () => {
    ...
    await waitFor(() =>
      expect(mocked.isPurchased).toHaveBeenCalledWith("advanced-stats", { target: "ollydigital.com" }),
    );
  });
...
describe("Purchase surface", () => {
  ...
  it("re-asks the host after checkout instead of trusting the returned flag", async () => {
    ...
    await waitFor(() => expect(mocked.buyProduct).toHaveBeenCalledWith("advanced-stats", { target: "ollydigital.com" }));
